...
void timer_mock_test()
{
  ...
  "hal::mock::timer"_test = []() {
    ...
    // Exercise + Verify
    expect(false == mock.is_running().is_running);
    ...
    expect(expected_delay ==
           std::get<1>(mock.spy_schedule.call_history().at(0)));
    expect(true == mock.is_running().is_running);
